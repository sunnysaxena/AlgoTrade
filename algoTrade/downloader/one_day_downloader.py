...
from backup.db_connection import get_mysql_connection
...
start_date = date(2005, 1, 3)  # (2018, 1, 1), 6
end_date = date(2005, 4, 10)  # (2018, 4, 10), 9
...
for _ in range(loop):
    get_history_data(start_date, end_date, '1D', 'BSE:SENSEX-INDEX')
    time.sleep(1)
    start_date = start_date + timedelta(days=100)
    end_date = end_date + timedelta(days=100)

hist_data.to_sql(name='sensex_1d', con=get_mysql_connection(), index=False, if_exists='append')
