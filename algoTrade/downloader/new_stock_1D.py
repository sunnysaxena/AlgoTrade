...
def store_new_stock():
    ...
    try:
        ...
        df.to_csv(str(datetime.now().time()) + '.csv', index=False)
        df.to_sql(name='sensex_1d', con=get_mysql_connection(), index=False, if_exists='append')
        print(df.head())
        print(df.tail())
        time.sleep(2)
        print('sensex_1d is updated...')
