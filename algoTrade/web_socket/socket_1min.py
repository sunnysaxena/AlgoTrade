                    ...
                    if new_ohlc_list:
                        ...
                        if len(ohlc_df) >= 20:
                            try:
                                ...
                                # Buy condition
                                if (prev_row['ema_short'] < prev_row['ema_long'] and last_row['ema_short'] > last_row['ema_long'] and
                                        ohlc_df['RSI'].iloc[-1] > 50 and last_row['MACD'] > last_row['Signal_Line']):
                                    if position != "BUY":
                                ...
                                # Sell condition
                                elif (prev_row['ema_short'] > prev_row['ema_long'] and last_row['ema_short'] < last_row['ema_long'] and
                                        ohlc_df['RSI'].iloc[-1] < 50 and last_row['MACD'] < last_row['Signal_Line']):
                                    if position != "SELL":
                        ...
                        logging.info(ohlc_df1[['timestamp', 'close', 'ema_short', 'ema_long', 'RSI', 'MACD', 'Signal_Line']])
